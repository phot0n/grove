...
import grove
from grove.pathway.run import redact

SECRET = "a05749bce74d4e162f98339d3889f3483052d39957a0f3fc"

ROW_DOCTYPE = Path(grove.__file__).parent / "grove/doctype/pathway_sync_row/pathway_sync_row.json"
...
class TestRedaction(unittest.TestCase):
	...
	def test_a_blank_secret_is_left_alone(self):
		...
		self.assertEqual(redact({"internal_key": ""})["internal_key"], "")
...
class TestEveryRowNamesTheBoxItIsAbout(unittest.TestCase):
	...
	def rows_written(self):
		for source in sorted((Path(grove.__file__).parent / "pathway").glob("*.py")):
			for literal in re.findall(r'\{("server_type":[^}]*)\}', source.read_text()):
				yield source.name, set(re.findall(r'"(\w+)"\s*:', literal))
