...
Pathway Sync grows on a timer, not on use: two scheduled runs each write a doc every two minutes
...
from grove.grove.doctype.pathway_sync.pathway_sync import PathwaySync
...
class TestClearOldLogs(IntegrationTestCase):
	def run_aged(self, days_old, server="gw-retention-test"):
		...
		frappe.db.set_value("Pathway Sync", doc.name, "creation", aged, update_modified=False)
		return doc.name

	def rows_of(self, name):
		return frappe.db.count("Pathway Sync Row", {"parent": name, "parenttype": "Pathway Sync"})

	def test_a_run_past_the_window_goes(self):
		old = self.run_aged(90)
		self.assertTrue(self.rows_of(old))
		PathwaySync.clear_old_logs(60)
		self.assertFalse(frappe.db.exists("Pathway Sync", old))
	...
	def test_a_recent_run_is_left_alone(self):
		...
		PathwaySync.clear_old_logs(60)
		self.assertTrue(frappe.db.exists("Pathway Sync", recent))
	...
	def test_the_window_is_what_the_caller_says(self):
		...
		PathwaySync.clear_old_logs(60)
		self.assertTrue(frappe.db.exists("Pathway Sync", run))
		PathwaySync.clear_old_logs(7)
		self.assertFalse(frappe.db.exists("Pathway Sync", run))

	def test_it_is_wired_up_for_log_settings(self):
		# Without the hook, Log Settings refuses the doctype and nothing ever clears.
		self.assertEqual(frappe.get_hooks("default_log_clearing_doctypes", {}).get("Pathway Sync"), [60])
