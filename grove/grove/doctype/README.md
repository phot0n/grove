...
| `Lost Usage` | A drained payload a pull could not record (one key, or a whole box), with the traceback. Replayed hourly through the normal pull path on the day it was drained; `replayed`, `attempts`, `last_error` say where it stands. A pending row is never deleted; Log Settings clears replayed ones after 90 days. |
