...
| `GPU Claim` | Who **holds** one card, named `<machine>:<gpu index>` so the primary key is the mutex — a second claim on a held card cannot be inserted, which is what stops two engines sharing a GPU. Named for the Machine because that is what the card belongs to, and `Inference Server.machine` is not unique. Held for `Draft`/`Provisioning`/`Active`/`Broken`, released on `Inactive`/`Terminated`. See `grove/placement/README.md`. |
