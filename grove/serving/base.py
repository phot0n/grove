...
Nothing here imports frappe, so an engine can be built and asserted on without a site. What is
concrete is placement arithmetic and Model facts, not engine policy."""
...
class EngineError(Exception):
	"""Raised, never thrown: the doctype layer turns it into frappe.throw."""


# A context length is a power of two: 128k is 131072, not 128000 — the number config.json declares
# and the only one vLLM accepts without the long-context override.
...
class Engine(ABC):
	...
	def __init__(
		...
		# The box's weakest card. 0 is "unknown" and skips the capability checks rather than
		# failing them, the same reading a blank CPU architecture gets.
		...
		# Bytes, learned off a profiled boot by the replica. 0 = not learned, vLLM profiles.
		self.kv_cache_memory = int(kv_cache_memory or 0)
		self.max_model_len = parse_context_length(max_model_len) or DEFAULT_MAX_MODEL_LEN
		# 0 = leave it to vLLM. Not defaulted: the right number depends on chunked prefill and
		# the model kind, and nothing reads it back the way the gateway reads the sequence cap.
		self.max_num_batched_tokens = int(max_num_batched_tokens or 0)
		# 0 = leave it to vLLM; the routing side then assumes default_concurrency.
		...
		# Split here, not at the placement, so both planes hand over the same raw string.
		self.startup_command = shlex.split(startup_command or "")
		# Off the Engine Image, not the placement: which request proves the engine serves is a
	...
	def tensor_parallel_size(self):
		"""GPUs left once the pipeline stages are carved out. World size is TP x PP, which has to
		equal the GPUs on hand."""
	...
	def is_streaming(self):
		"""Weights come off the S3 mirror, so the bucket's credentials belong in the environment
		and nothing needs pre-downloading."""
	...
	@abstractmethod
	def repo(self):
		"""The positional argument the image is invoked with, "" when its entrypoint is the whole
		story. Rendered UNQUOTED into the run script, so never operator free-text."""

	@property
	@abstractmethod
	def args(self):
		"""The flag list that follows the positional, each element one quoted argument."""

	@property
	@abstractmethod
	def command(self):
		"""Positional + flags as one shell-safe string. Never the image itself: the placement
		supplies that."""
	...
	def env(self, hf_home="", cache_root="", api_key="", hf_token="", streaming_env=None):
		...
		the PLACEMENT names the paths; a blank path leaves the variable out rather than guessing.
	...
	def placement_errors(self):
		...
		does not fail minutes in, on the box. An engine that cannot judge the shape returns []
		rather than inventing a rule."""

	@property
	@abstractmethod
	def health_path(self):
		"""The path that answers 200 once this engine is up, "" when it publishes none and the
		provider's own state is the whole status."""

	@property
	@abstractmethod
	def warmup_request(self):
		"""The smallest real inference this engine can serve → {"path": ..., "body": {...}}. Proof
		of a forward pass under the name the gateway routes on, which a health path does not give.
		{} turns the step off on both planes.
	...
	def has_api_key(self):
		"""Whether Grove mints the key this engine serves behind. False means the image owns its
		own auth, and a key of ours would be sent as a bearer to something that never asked."""
...
def build_engine(engine_kind, model_name, model, **tuning):
	"""One placement's Engine. An unknown knob is a TypeError here rather than a flag that
